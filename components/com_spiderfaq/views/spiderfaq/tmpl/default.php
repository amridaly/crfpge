...
$standcat = (int)JRequest::getVar('standcat');
$categories = (int)JRequest::getVar('faq_cat');
$standcategories=(int)JRequest::getVar('standcatid');
...
$query = "SELECT * FROM #__spiderfaq
  WHERE published= '1' AND category=".$db->escape($categories)." AND (title LIKE '%".$db->escape($_POST['search'.$random])."%' OR article LIKE '%".$db->escape($_POST['search'.$random])."%' OR fullarticle LIKE '%".$db->escape($_POST['search'.$random])."%')";
...
$query1 = "SELECT * FROM #__content
  WHERE state='1' AND catid=".$db->escape($standcategories)." AND (title LIKE '%".$db->escape($_POST['search'.$random])."%' OR introtext LIKE '%".$db->escape($_POST['search'.$random])."%' OR `fulltext` LIKE '%".$db->escape($_POST['search'.$random])."%')";
...
$(window).load(function(){  $('#id_for_com .post_exp')[0].click();  faq_changeexp<?php echo $random ?>();}) 
</script>
<?php
}
?>	
...
if(count($rows))
{
 echo '<div class="expcoll" id="expcol'.$random. '">
     <a class="post_exp" id="post_expcol'.$random. '"><span onclick="faq_changeexp'.$random.'()">'.JText::_("EXPANDALL").' </span></a><span>|</span>
     <a  class="post_coll" id="post_expcol'.$random. '"><span onclick=" faq_changecoll'.$random.'()">'.JText::_("COLLAPSEALL").'</span></a></div>';
...
if ($show_title=="")
	...
	if($show_title==1) 
	{
			...
			echo '</li><li id="post-1236" class="selected" style="margin-left:'.$stl->marginleft.'px"><div class="post_top">
				  <div class="post_right" id="post_right'.$random.'">
					  <a href="#" class="post_ajax_title"><span onclick="faq_changesrc'.$random.'('.$i.')"><h2 class="post_title" id="post_title'.$random.'">'?>
					  <?php if ($stl->tchangeimage1!=""){ echo'<div class="tchangeimg" id="tchangeimg'.$random.'"><img src="administrator/'.$stl->tchangeimage1.'"  id="stl'.$random.$i.'" /></div>'  ?><?php } echo '<div class="ttext" id="ttext'.$random.'">'.$p.'. '.$row->title.'</div></h2></span></a>
				    </div>
			    </div>';
			...
			if (strlen($row->fulltext)>1){
				  ...
				  <div class="post_content_wrapper" id="post_content_wrapper'.$random.'" style="'?><?php if($stl->abg==1) {  if ($stl->abgimage!="") { echo 'background-image:url(administrator/'.$stl->abgimage.')'?><?php } echo '">' ?><?php } else echo 'background-color:#'.$stl->abgcolor.'">
				    '?><?php if ($stl->aimage!=""){ echo'<div class="imgbefore" id="imgbefore'.$random. '"><img src="administrator/'.$stl->aimage.'"  /></div>'  ?><?php } echo '<div class="post_right" id="post_right'.$random.'"><div class="atext" id="atext'.$random. '">'.$row->introtext.'<p><a href="#" class="more-link'.$random. '">More</a></p>
	...
	else 
		{
			echo '</li><li id="post-1236" class="selected" style="margin-left:'.$stl->marginleft.'px"><div class="post_top">
				  <div class="post_right" id="post_right'.$random.'">
					  <a href="#" class="post_ajax_title"><span onclick="faq_changesrc'.$random.'('.$i.')"><h2 class="post_title" id="post_title'.$random.'">
					  '?><?php if ($stl->tchangeimage1!=""){ echo'<div class="tchangeimg" id="tchangeimg'.$random.'"><img src="administrator/'.$stl->tchangeimage1.'"  id="stl'.$random.$i.'" /></div>'  ?><?php } echo '<div class="ttext" id="ttext'.$random.'">'.$p.'. </div></h2></span></a>
				    </div>
			    </div>';
	...
	if (strlen($row->fulltext)>1){
					  echo '<div class="post_content">
				  <div class="post_content_wrapper" style="'?><?php if($stl->abg==1) {  if ($stl->abgimage!="") { echo 'background-image:url(administrator/'.$stl->abgimage.')'?><?php } echo '">' ?><?php } else echo 'background-color:#'.$stl->abgcolor.'">
				    '?><?php if ($stl->aimage!=""){ echo'<div class="imgbefore" id="imgbefore'.$random. '"><img src="administrator/'.$stl->aimage.'"  /></div>' ?><?php } echo '<div class="post_right" id="post_right'.$random.'"><div class="atext" id="atext'.$random. '">'.$row->introtext.'<p><a href="#" class="more-link'.$random. '">More</a></p>
			            <div class="post_content_more" style="margin-top:-6px;">'.$row->fulltext.'</div>	    
			               </div></div>'?><?php if ($stl->aimage2!=""){ echo'<div class="imgafter"><img src="administrator/'.$stl->aimage2.'"  /></div>'  ?><?php } echo '
			       </div></div>
				</li>';
			}
			else{
...
$row = &$rows[$i];
	...
	if (strlen($row->fullarticle)>1){
					  echo '<div class="post_content">
				  <div class="post_content_wrapper" style="'?><?php if($stl->abg==1) {  if ($stl->abgimage!="") { echo 'background-image:url(administrator/'.$stl->abgimage.')'?><?php } echo '">' ?><?php } else echo 'background-color:#'.$stl->abgcolor.'">
				    '?><?php if ($stl->aimage!=""){ echo'<div class="imgbefore" id="imgbefore'.$random. '"><img src="administrator/'.$stl->aimage.'"  /></div>'  ?><?php } echo '<div class="post_right" id="post_right'.$random.'"><div class="atext" id="atext'.$random. '">'.$row->article.'<p><a href="#" class="more-link'.$random. '">More</a></p>
			...
			echo '<div class="post_content">
				  <div class="post_content_wrapper" id="post_content_wrapper'.$random.'" style="'?><?php if($stl->abg==1) {  if ($stl->abgimage!="") { echo 'background-image:url(administrator/'.$stl->abgimage.')'?><?php } echo '">' ?><?php } else echo 'background-color:#'.$stl->abgcolor.'">
				    '?><?php if ($stl->aimage!=""){ echo'<div class="imgbefore" id="imgbefore'.$random. '"><img src="administrator/'.$stl->aimage.'"  /></div>'  ?><?php } echo '<div class="post_right" id="post_right'.$random.'">'?><?php if ($row->article!=""){ echo '<div class="atext" id="atext'.$random. '">'.$row->article.'</div>'?><?php } else echo '<div style="margin-top:50px">
					</div></div>'?><?php if ($stl->aimage2!=""){ echo'<div class="imgafter" id="imgafter'.$random. '"><img src="administrator/'.$stl->aimage2.'"  /></div>'  ?><?php } echo '
			       </div></div>
				</li>';
			}
