...
/**
    ...
    public function delete($cid = array())
    {
        ...
        if ($total > 0)
        {
            // default template can not erase - with exception the special layouts from type: 'Standard Categories Layout for paginated Subcategories'!
            for( $i=0; $i < $total; $i++ ) {
                $db->setQuery("SELECT locked FROM #__jdownloads_templates WHERE id = ($cid[$i])");
                if ($db->loadResult() == 1 ) {
                    // check which type we have
                    $db->setQuery("SELECT * FROM #__jdownloads_templates WHERE id = ($cid[$i])");
                    $row = $db->loadObject();
                    if ($row->template_typ == '1' && $row->use_to_view_subcats == '0'){
