...
/*    public function getItem($pk = null){
    ...
    protected function batchMove($value, $pks, $dummy = 0)
    {
        ...
        foreach ($pks as $pk)
        {
            ...
            if ($table->cat_id == 1){
                // Uncategorisied Download
                $source_path = $jlistConfig['files.uploaddir'].'/_uncategorised_files/'.$table->url_download;
            } else {
                $oldCategoryTable = JTable::getInstance('category', 'jdownloadsTable');
                if (!$oldCategoryTable->load($table->cat_id)) {
                ...
                $source_path = $jlistConfig['files.uploaddir'].'/'.$source_folder.'/'.$table->url_download;
            }
            // Set the new category ID
            $table->cat_id = $new_category_id;
            ...
            if ($table->url_download){
                if ($new_category_id > 1){
                    ...
                    $target_path = $jlistConfig['files.uploaddir'].'/'.$target_folder.'/'.$table->url_download;
                } else {
                    $target_path = $jlistConfig['files.uploaddir'].'/_uncategorised_files/'.$table->url_download;
                }
