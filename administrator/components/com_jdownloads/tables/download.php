...
/**
    ...
    public function checkData($isNew, $auto_added = false)
    {
        ...
        if ($app->isSite() && !$auto_added){
            ...
            if ($user_rules->form_extra_date_1 && $user_rules->form_extra_date_1_x && $this->custom_field_11 == '0000-00-00 00:00:00') $this->setError(JText::_('COM_JDOWNLOADS_REQUIRED_CUSTOM_DATE')); 
            if ($user_rules->form_extra_date_2 && $user_rules->form_extra_date_2_x && $this->custom_field_12 == '0000-00-00 00:00:00') $this->setError(JText::_('COM_JDOWNLOADS_REQUIRED_CUSTOM_DATE')); 
            ...
            if ($this->date_added){
                $date_dum = new DateTime($this->date_added);             
                $this->date_added = $date_dum->format('Y-m-d H:i:s'); 
            }
            ...
            if ($this->publish_from){
                $date_dum = new DateTime($this->publish_from);             
                $this->publish_from = $date_dum->format('Y-m-d H:i:s'); 
            }
            
            if ($this->publish_to){
                $date_dum = new DateTime($this->publish_to);             
                $this->publish_to = $date_dum->format('Y-m-d H:i:s'); 
            }
            
            if ($this->modified_date){
                $date_dum = new DateTime($this->modified_date);             
                $this->modified_date = $date_dum->format('Y-m-d H:i:s'); 
            }

            if ($this->custom_field_11){
                $date_dum = new DateTime($this->custom_field_11);             
                $this->custom_field_11 = $date_dum->format('Y-m-d H:i:s');
            }
            
            if ($this->custom_field_12){
                $date_dum = new DateTime($this->custom_field_12);             
                $this->custom_field_12 = $date_dum->format('Y-m-d H:i:s');
            }
        ...
        if (!$auto_added){
            ...
            if($files['tmp_name']['file_upload'] != ''){
                ...
                $only_name = utf8_encode(JFile::stripExt($files['name']['file_upload']));
                $file_extension = JFile::getExt($files['name']['file_upload']);            
                
                // check filename
                $filename_new = JDownloadsHelper::getCleanFolderFileName($only_name).'.'.$file_extension;
                
                $only_name = utf8_encode(JFile::stripExt($filename_new));
                ...
                if ($only_name != ''){
                    ...
                    if(JFile::upload($files['tmp_name']['file_upload'], $target_path, false, true)) {
                       ...
                       if ($jlistConfig['create.pdf.thumbs'] && strtolower($file_extension) == 'pdf'){
                           ...
                           $picfilename     = utf8_encode(JFile::getName($target_path));
                           $only_name       = utf8_encode(JFile::stripExt($picfilename));
               ...
               if ($selected_updatefile){
                   ...
                   $only_name = utf8_encode(JFile::stripExt($selected_updatefile));
                   ...
                   if ($filename_renamed){
                       ...
                       $only_name = utf8_encode(JFile::stripExt($filename_new_name));
                   ...
                   if ($preview_filename_renamed){
                       ...
                       $only_name = utf8_encode(JFile::stripExt($preview_filename_new_name));
                       ...
                       $only_name = utf8_encode(JFile::stripExt($preview_filename_new));
            ...
            if($files['tmp_name']['preview_file_upload'] != ''){
                ...
                $only_name = utf8_encode(JFile::stripExt($files['name']['preview_file_upload']));
            ...
            if ($sum > 0){
                ...
                for ($i=0; $i < $sum; $i++){
                    ...
                    if ($filename != '' && JDownloadsHelper::fileIsImage($temptype)){     
                        ...
                        $filename = utf8_encode(JDownloadsHelper::getCleanFolderFileName($only_name)).'.'.$file_extension;
