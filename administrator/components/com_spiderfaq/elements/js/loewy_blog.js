...
(function($){
	...
	$().ready(function(){
		...
		//tags button - show
		$("a.button_tags").each(function(){
			var self = this;
			var parent = $(this).parent();
			var menu = $(".post_tags",parent);
		...
		$(".archives").each(function() {
			if ($("> div.odd", this).length) {
			$(".archives > div.odd").after('<div class="clear"></div>');
			}
		});
					...
					if (href != "") {
							...
							if (li.is(".post_content_opened")) {
								...
								$(".post_content_more", li).animate({height:"show"}, {
									queue:false, duration:100, easing: "easeInOutExpo", complete:function(){
										posts_waiting = false;
		...
		$("a.post_coll", this).each(function(){
				...
				$(this).bind("click", function(){
					...
					if (posts_waiting) return false;
						...
						if ($('ul.posts').children().is(".post_ajax_loaded")) {
							if ($('ul.posts').children().is(".post_content_opened")) {
								$('ul.posts').children().removeClass("selected");
								$(".post_content", $('ul.posts').children()).animate({height:"hide"}, {
									 complete:function(){
										posts_waiting = false;
										$('ul.posts').children().removeClass("post_content_opened");
									}
								});
							}
							}
		...
		$("a.post_exp", this).each(function(){
				...
				$(this).bind("click", function(){
						...
						if ($('ul.posts').children().is(".post_ajax_loaded")) {
						$('ul.posts').children().addClass("selected");
								$(".post_content", $('ul.posts').children()).animate({height:"show"}, {
									duration:300, complete:function(){
										posts_waiting = false;
										$('ul.posts').children().addClass("post_content_opened");
									}
								});
							}
