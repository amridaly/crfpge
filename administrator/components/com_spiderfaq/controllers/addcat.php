...
class SpiderfaqControllerAddcat extends JControllerLegacy
{
...
'limitstart', 0, 'int');
	...
	$where 		= ( count( $where ) ? ' WHERE ' . implode( ' AND ', $where ). 'AND published="1"' : ' WHERE published="1"' );
	if ($filter_order == 'id'){
	...
	// search filter	
		...
		require_once JPATH_COMPONENT.'/views/add.php';
		SpiderfaqViewAddcat::addcat( $rows, $pageNav, $lists );
