...
class SpiderfaqViewAddcat
{


	function addcat( &$rows, &$pageNav, &$lists )
	{
...
function xxx()
{
	...
	window.parent.jSelectCategories(id, title);
		...
		<form action="index.php?option=com_spiderfaq&c=addcat&tmpl=component" method="post" name="adminForm" id="adminForm">
		...
		<table>
		 <tr>
            ...
            <button onclick="xxx();" style="width:98px; height:34px; background:url(components/com_spiderfaq/images/add_but.png) no-repeat;border:none;cursor:pointer;">&nbsp;</button>           
             </td>
		</tr>
		</table>  
...
?>
        <tr class="<?php echo "row$k"; ?>">
            ...
            <input type="hidden" id="title_<?php echo $i?>" value="<?php echo  htmlspecialchars($row->cattitle);?>" />
            
            </td>
        	<td align="center"><?php echo $row->id?></td>
        	<td><a style="cursor: pointer;" onclick="window.parent.jSelectCategories(['<?php echo $row->id?>'],['<?php echo htmlspecialchars(addslashes($row->cattitle));?>'])"><?php echo $row->cattitle?></a></td>            
    ...
    <input type="hidden" name="c" value="addcat" />
