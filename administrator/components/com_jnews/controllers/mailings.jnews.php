...
//			$mailing->frombounce = ( !isset( $mailing->frombounce ) || empty( $mailing->frombounce ) ) ? $GLOBALS[JNEWS.'sendmail_from'] : $mailing->frombounce;
		...
		case ('add'):
				...
				$mailingType = JRequest::getInt('listype' );
				if( empty($listId) ) $listId = JRequest::getInt('listid',0);
				...
				$total++;
				jNews_Tools::redirect('index.php?option='.JNEWS_OPTION.'&act=mailing&task=edit&mailingid=0&issue_nb='.$total.'&listype='.$mailingType.'&listid=' . (int)$listId . '&isEdit=0');//mariap
		...
		case ('send'):
			...
			$linkURL = jNews_Tools::completeLink( 'option='.JNEWS_OPTION.'&act=mailing&task=continuesend&mailingid=' . (int)$mailingId . '&totalsend='.$totalSub, true, false, true );
...
//			$queueC->updateSenddateToNow($mailingId);
		...
		case ('testspam'):
			...
			$link = 'https://joobi.co/index.php?option=com_jlinks&controller=redirect&link=SpamCheck&alt=jnewsdoc_glossary';
		...
		case ('preview'):
			...
			$emailaddress = jnews::getVar('emailaddress', '');
          ...
          if($archivemailing->html==1){
			...
			if($mailingId==0) JRequest::getInt('mailingid',$mailingId);
			break;
		...
		case ('deletequeue');
			...
			if ( !empty($mailingID) ) {
				$db = JFactory::getDBO();
				$db->setQuery( 'DELETE FROM `#__jnews_queue` WHERE `mailing_id`=' . (int)$mailingID );
				$db->query();
	...
	if ($showMailings){
		if(empty($listType)) $listType=JRequest::getInt('listype', 0);
		if(empty($listId)) $listId=JRequest::getInt('listid', 0);
...
function checkTag(){
	...
	$alt_content = JRequest::getVar('textcontent_', '', null, 'STRING' );
