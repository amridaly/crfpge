	 ...
	 public static function showListingLists($lists, $action, $task, $forms, $show, $listsearch='', $limit=0, $setLimit=null, $totalSubs=null, $setSort=null, $totalUnSubs=null ) {
		...
		<tr class="row<?php echo ($i + 2) % 2; ?>">
			...
			<?php if($mainframe->isAdmin()){ ?>
				<a href="<?php
				$act = jnews::getVar('act', 'list', '', 'WORD' );
	...
	<?php } else { echo '<input type="hidden" name="hidden" value="' .$listEdit->hidden .'" />'; } ?>
		...
		if($listType == 2){
	 	...
	 	<tr>
			...
			<?php
				require_once(JNEWS_JPATH_ROOT_NO_ADMIN .DIRECTORY_SEPARATOR.'administrator'.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.JNEWS_OPTION.DIRECTORY_SEPARATOR.'views'.DIRECTORY_SEPARATOR.'mailings.jnews.html.php');
