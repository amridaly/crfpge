...
//		$img = 'newsletter.png';
    	...
    	$hidden = '<input type="hidden" name="listid" value="' . (int)$listId . '" />';
		...
		<thead>
			<tr>
				...
				if ( empty($listType) || $listType ==0 ) {
					$listType = JRequest::getInt('listype', 0);//listType to listype
				}
	...
	<tr class="row<?php echo ($i + 1) % 2;?>">
		...
		if ((!$show['admin']) OR ( $mailing->published == 1 AND ($mailing->mailing_type == 1 or $mailing->mailing_type == 7 or $mailing->mailing_type == 2))) {
			$link = 'option='.JNEWS_OPTION.'&act=' . $action . '&task=view&listid=' . (int)$listId . '&mailingid=' .$mailing->id . '&listype=' .$mailing->mailing_type . $item ;
		} else {
		...
		<td align="center"><center>
			<a href="<?php
			$listidmailing = JRequest::getInt('listid');
...
<script language="javascript" type="text/javascript">
	...
	echo jnews::setLegend();
	 ...
	 public static function editMailing($mailingEdit, $new, $listId, $forms, $show, $isEdit=false) {
		...
		$mailingIdchk=JRequest::getInt('mailingid',0);
...
?>
	...
	<table class="<?php echo jnews::myTheme(); ?>">
		...
		<?php
			...
			$k = 0;
			$i = 0;
			//$listId=jnews::getVar('listId');

			if( empty($listId) ) $listId=JRequest::getInt('listid');
		...
		?>
				<tr class="<?php echo "row$k"; ?>">
					...
					<td>
						...
						$getMailingId = ( !empty($mailingEdit->id) ) ? $mailingEdit->id : JRequest::getInt('mailingid' );
						$result = jNews_Mailing::mailingListFound( $getMailingId, $list->id );
						$result = ( $result ) ? 1 : 0;
						if( $result == 0 ){
							$lstID = JRequest::getInt('listid',0);
		...
		<table class="jnewstabletable" cellspacing="1" border="0">
			<tbody>
				<tr>
					...
					<?php if (jnews::getVar('act', '', '', 'WORD' )=='mailing' AND JRequest::getInt('listype')==2 /*$show['delay']*/) { ?>
							<tr>
								<td class="key">
									<span class="editlinktip">
									<?php
								...
								<td>
								<?php
								$mailid = JRequest::getInt('mailingid' );
					...
					<?php } ?>
					<?php if (jnews::getVar('act', '', '', 'WORD' )=='mailing' AND JRequest::getInt('listype')==1 AND class_exists('jNews_Auto')) { ?>
						<tr>
							<td class="key">
								<span class="editlinktip">
								<?php
...
<?php
	...
	$doc = JFactory::getDocument();
		...
		if(sizeof($files) > 0){
			...
			if( version_compare(JVERSION,'1.6.0','<') ){
				$delImgPath = JNEWS_JPATH_LIVE .DIRECTORY_SEPARATOR. 'administrator' .DIRECTORY_SEPARATOR. 'images' .DIRECTORY_SEPARATOR. 'delete_f2.png';
			}else{
				$delImgPath = JNEWS_JPATH_LIVE .DIRECTORY_SEPARATOR. 'administrator' .DIRECTORY_SEPARATOR. 'components' .DIRECTORY_SEPARATOR. JNEWS_OPTION .DIRECTORY_SEPARATOR. 'images' .DIRECTORY_SEPARATOR. 'delete_queue.png';
			}
		...
		<tr>
			...
			<td align="left">
				...
				foreach ($mailing->attachments as $file) {
					echo '<a href="'.JNEWS_JPATH_LIVE.$GLOBALS[JNEWS.'upload_url'].DIRECTORY_SEPARATOR.basename($file).'" target="_blank">'.basename($file).'</a><br />';
				}
				?>
			</td>
		</tr>
		...
		$year = intval(date('Y'));
		 if(!empty($jcalcats)){
		 	foreach($jcalcats as $jcalcat){
				 $query = "SELECT `extid`, `title` ,`start_date` FROM #__jcalpro2_events where `cat` = " . (int)$jcalcat->cat_id." AND (`start_date` >= '".$year."' OR `end_date` >= '".$year."' ) ORDER BY `start_date` DESC";
				 $db->setQuery($query);
