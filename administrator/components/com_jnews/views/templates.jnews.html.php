			...
			?>
				<tr class="row<?php echo ($i + 1) % 2;?>">
					...
					<td align="center">
						<span class="editlinktip">
							...
							if(strpos($filename[0],'entwine') !== false){
								...
								$popimgpath='https://joobi.co/images/newsletter_templates/'.$filename[0].'/'.$filenamecolor[0].'/'.$tmblfile;
							}else{
									$popimgpath='https://joobi.co/images/newsletter_templates/'.$filename[0].'/'.$tmblfile;
							}
					...
					if($template->availability == 0) {
						...
						$link='https://joobi.co/index.php?option=com_jlinks&controller=redirect&link=newsletter_template_'.$namekey[0];
						echo '<a href="'.$link.'">'._JNEWS_TEMPLATE_DWN.'</a>';
					}
					if($template->availability== -1){
						$link='https://joobi.co/index.php?option=com_jlinks&controller=redirect&link=newsletter_template_'.$template->namekey;
...
<!--                </form>-->
	...
	public static function createTemplate( &$template, $form ) {
		...
		@$mediacomjNewsTemplatesdir = JNEWS_JPATH_ROOT_NO_ADMIN .DIRECTORY_SEPARATOR. 'media'.DIRECTORY_SEPARATOR. JNEWS_OPTION .DIRECTORY_SEPARATOR. 'templates'.DIRECTORY_SEPARATOR.$template->namekey;
		@chmod(JNEWS_JPATH_ROOT_NO_ADMIN .DIRECTORY_SEPARATOR. 'media'.DIRECTORY_SEPARATOR. JNEWS_OPTION .DIRECTORY_SEPARATOR. 'templates'.DIRECTORY_SEPARATOR.$template->namekey, 0755);
