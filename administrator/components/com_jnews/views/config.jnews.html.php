...
class jNews_ConfigHTML {
	...
	<table class="jnewstable" cellspacing="1">
	   ...
	   if( class_exists('jNews_Social') ) { ?>
		...
		<?php if ( @include_once( JNEWSPATH_ADMIN . 'social' .DIRECTORY_SEPARATOR. 'class.social.php' ) ) {
			if( class_exists('jNews_Social') ) { ?>
		<tr>
			<td width="185" class="key">
				<span class="editlinktip">
...
//							else $value = 0;
	...
	<!--	<tr>
		...
		<?php } if($GLOBALS[JNEWS.'level'] > 2) {
			...
			if( JFolder::exists(JNEWS_JPATH_ROOT_NO_ADMIN .DIRECTORY_SEPARATOR. 'administrator' .DIRECTORY_SEPARATOR. 'components' .DIRECTORY_SEPARATOR. 'com_jsubscription' )
				|| JFolder::exists(JNEWS_JPATH_ROOT_NO_ADMIN .DIRECTORY_SEPARATOR. 'components' .DIRECTORY_SEPARATOR. 'com_jsubscription' )){ ?>
		<tr>
			<td width="185" class="key">
				<span class="editlinktip">
				<?php
