...
$jNewsMainFrame->registerEvent( 'jnewsbot_transformall', 'loadvmproduct' );
 ...
 function virtuemartproduct($forms, $params = null){
		...
		$vmproductsearch = JRequest::getVar('vmproductsearch', '', '', 'WORD' );
		echo $forms['main'];
...
class virtuemartProducts {
	...
	function _getVMProducts($vmproductsearch='', $setLimit = null){
		...
		if (!is_null($setLimit) && $setLimit->start != -1 && $setLimit->end) $query .= ' LIMIT ' . (int)$setLimit->start . ', ' . (int)$setLimit->end;
		$db->setQuery($query);
...
//		$query .= 'LEFT JOIN `#__vm_tax_rate` as c on a.product_tax_id = c.tax_rate_id ';
		$query .= 'WHERE a.virtuemart_product_id=' . (int)$productId.' LIMIT 1';
		$db = JFactory::getDBO();
		$db->setQuery($query);
		...
		if(!empty($allresults[3][$i])){
			...
			if(!empty($langid)){
				$query = "SELECT reference_field, value FROM `#__jf_content` WHERE `published` = 1 AND `reference_table` = 'virtuemart_products' AND `language_id` = $langid AND `reference_id` = " . (int)$productId;
				$db->setQuery($query);
