...
//				}//endallow_unregistered
				...
				if($GLOBALS[JNEWS.'level'] > 1){//check the version is plus or pro
					if(empty($esc) && $this->enable_captcha){//check if $esc has been initialized
					    ...
					    include_once JPATH_BASE.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_jnews'.DIRECTORY_SEPARATOR.'captcha' .DIRECTORY_SEPARATOR.'generateText.php';
		...
		$HTML .= '<div style="display:none" ><a href="https://joobi.co">Joomla Extensions</a> powered by Joobi</div>';
	    return $HTML;
...
/*<p>function to show captcha </p>
	...
	*/
	function _showCaptcha($code){
		...
		if( $this->enable_captcha)   //if captcha is enabled
		{
			if (extension_loaded('gd') AND extension_loaded('mcrypt'))
			{
				...
				if($gdinfo['FreeType Support']){
					...
					$captchaHTML .= '<img alt="catchme" style="border: 0px;" src="'. $path .'&changetext=1" id="captcha_image'.$this->num.'" />';
        ...
        function _showCaptchaHidden( $code, $version=2 ) {
...
/**
		...
		* @param subscriber subscriber info of a logged in user
        ...
        function _addCSS() {
        	...
        	//Start - Subhadip Khan modification (6th June 2015, 4PM IST) for joomla none selected theme for jnews module
        	if($this->cssfile == '-1') $this->cssfile = 'default.css';
        	//End - Subhadip Khan modification for
