...
*/
	...
	switch( $name ) {
		...
		case 'mailings' :
				...
				$query = "SELECT `list_type`,`follow_up`,`cat_id`,`delay_min`,`delay_max`,`notify_id`,`next_date`,`start_date` FROM `#__acajoom_lists` WHERE `id` =" . (int)$listId;
...
*/
	...
	*/
	...
	function _selectUpdate($akey){
		...
		$db = JFactory::getDBO();
		$query = 'SELECT `value` FROM `#__acajoom_xonfig` WHERE `akey`=\''.$db->escape($akey).'\'';
		$query =stripslashes($query);
		$db->setQuery($query);
		$act=$db->loadResult();
		...
		$query = 'UPDATE `#__jnews_xonfig` set `value`=' . $db->escape($act) .' WHERE `akey`=\''.$db->escape($akey).'\'';
		$db->setQuery($query);
		$db->query();
